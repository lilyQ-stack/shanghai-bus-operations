...
def load_shmaas(date):
 files=[p for p in sorted((ROOT/'data/shmaas').glob(f'{date}-*.jsonl')) if not p.name.endswith('-reverse-watch.jsonl')];snaps=[]
 for p in files:snaps+=load_jsonl(p)
 snaps=[s for s in snaps if s.get('success') and s.get('sample_time_cst') and s.get('route')];snaps.sort(key=lambda s:t(s['sample_time_cst']));return snaps,[p.name for p in files]
def build(date,snaps):
 ...
 for s in snaps:
  ...
  for d in s.get('directions',[]) or []:dirs[(route,int(d.get('direction',0)))]={'start':str(d.get('start_stop') or ''),'end':str(d.get('end_stop') or ''),'stop_count':int(d.get('stop_count') or 0)}
 ...
 # Critical trip isolation: evidence for one dispatch ends at the next captured dispatch
 # of the same plate. This prevents a later trip from proving an earlier trip short-turn.
 ...
 def trip_end(route,plate,dep):
  later=[x for x in vehicle_departures[(route,plate)] if x>dep]
  return min(later) if later else dep+timedelta(hours=4)
 def terminal_eta(route,ev,direction,dep,endtime):
  info=dirs.get((route,direction),{});end=info.get('end','');n=info.get('stop_count',0);vals=[]
  for e in ev:
   if e['direction']!=direction or not(dep<=e['time']<endtime) or e['role'] not in {'current','next'}:continue
   ...
   if dep<=pred<=min(endtime,dep+timedelta(hours=4)):vals.append((e,pred,dist))
 ...
 for d in sorted(dispatch.values(),key=lambda x:(x['route'],x['departure'],x['plate'])):
  ...
  eta=terminal_eta(route,ev,direction,dep,endtime) if dep else None;arrival,note,u,method=(hm(eta[0]),eta[1],eta[2],eta[3]) if eta else('待确认','尚无满足±5分钟标准的终点ETA','',None)
  ce=[Event(e['time'],e['direction'],e['role'],e['stop_seq'],e['stop_name'],e['arrive_minutes'],e['remaining_stops']) for e in ev];cls=classify_short_turn(events=ce,direction=direction,dep_dt=dep,end_stop=info.get('end',''),stop_count=info.get('stop_count',0),terminal_eta_confirmed=method in {'terminal_eta_near','terminal_eta_consensus'}) if dep else None;stype=cls.service_type if cls else'全程车';reason=cls.reason if cls else'无有效发车时间';duration='待确认'
...
def main():
 ...
 rows,plates,times,dc=build(date,snaps);out=ROOT/'data/export';out.mkdir(parents=True,exist_ok=True);fields=['临时车辆编号','车牌号','方向','始发站','发车时间','终点站','班次类型','班次类型判定依据','到达时间','全程时间','车速（km/h）','当日平均车速（km/h）','当日车速排名','到达时间说明','到达误差估计（分钟）','本车首次观测','本车末次观测','数据源'];by=defaultdict(list)
 ...
 write_csv(out/f'{date}-operations.csv',['线路']+fields,sorted(rows,key=lambda x:(x['线路'],x['发车时间'],x['车牌号'])));meta={'date':date,'source_files':files,'shmaas_snapshot_count':len(snaps),'shmaas_dispatch_event_count':dc,'trip_row_count':len(rows),'service_type_status':'Trip-isolated evidence. Before 80% source progress + reverse physical gain >=2 stops => 疑似区间车; no hard elapsed-time limit; reliable same-direction terminal ETA guards full trips.','first_shmaas_snapshot':min(times).isoformat() if times else None,'last_shmaas_snapshot':max(times).isoformat() if times else None};(out/f'{date}-operations-meta.json').write_text(json.dumps(meta,ensure_ascii=False,indent=2),encoding='utf-8');print(f'Wrote {len(rows)} operations rows for {date}');return 0
