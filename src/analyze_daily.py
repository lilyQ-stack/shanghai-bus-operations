...
def trajectory_classification(route_info, vehicle_events, route, direction, dep, next_same_dep):
    info = route_info.get((route, direction), {})
    count = info.get("stop_count", 0)
    terminal = info.get("end", "")
    ...
    opp = [e for e in window if e["direction"] != direction and e["role"] in {"current", "next"} and e["eta"] is not None and (e["time"] - dep).total_seconds() / 60 >= 20]
    ...
    if not near_terminal and 0 <= gap <= 20:
        predicted = last_same["time"] + timedelta(minutes=last_same["eta"])
        return (
            "疑似区间车",
            f"原方向末次观测停留在中途站，{round(gap)}分钟后同车反向运行",
            last_same["stop_name"],
            fmt_hm(predicted),
        )
    if near_terminal:
    ...
    return "运行异常待查", "存在方向切换但证据不足以确认区间折返", last_same["stop_name"], ""
    ...
    for d in ordered:
        ...
        if explicit:
            ...
            service_type, service_basis, last_stop, last_eta = trajectory_classification(
                route_info, ve, route, direction, dep, next_same
            )
        ...
        rows.append({
            ...
            "全程时间": duration,
            "班次类型": service_type,
            "班次类型判定依据": service_basis,
            "到达置信度": grade,
            "到达估算方法": arrival_method,
...
def main():
    ...
    meta = {
        ...
        "short_turn_rule": "A suspected short turn requires a same-vehicle intermediate last observation followed by reliable reverse-direction running after at least 20 minutes and within 20 minutes of that last same-direction observation; near-terminal cases are downgraded to abnormal pending review.",
    }
